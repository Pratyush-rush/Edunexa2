...
export default function TeacherClassroomAnalytics() {
  ...
  async function handleAIQuery(text?: string) {
    ...
    try {
      ...
      const res = await fetch(apiUrl("/ai/analytics"), {
        method: "POST",
                    ...
                    {weakTopics.slice(0, 5).map((topic) => (
                      <div key={topic.topic} className="weak-topic-row">
                        ...
                        <div className="topic-rate-bar">
                          <div
                            className={`topic-rate-fill ${topic.needsAttention ? "fill-alert" : "fill-pass"
                              }`}
                ...
                <div className="weak-topics-list">
                  {weakTopics.map((t) => (
                    <div key={t.topic} className="weak-topic-row">
                      ...
                      <div className="topic-rate-bar">
                        <div
                          className={`topic-rate-fill ${t.needsAttention ? "fill-alert" : "fill-pass"
                            }`}
                  ...
                  <tbody>
                    {studentAnalytics.map((s) => (
                      <tr key={s.studentId}>
                        ...
                        <td>
                          <span
                            className={`badge-${s.status === "Excellence"
                              ? "pass"
                              : s.status === "At Risk"
                                ? "danger"
                                : s.status === "Needs Help"
          ...
          <div className="ai-copilot-card">
            ...
            <div className="copilot-messages">
              {aiMessages.map((msg) => (
                <div
                  key={msg.id}
                  className={`copilot-msg-row ${msg.role === "user" ? "user-row" : "assistant-row"
                    }`}
                >
                  {msg.role === "assistant" && (
