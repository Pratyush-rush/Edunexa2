...
export default function AdminDashboard() {
  ...
  async function loadAdminDashboard() {
    ...
    try {
      ...
      const { data: profile } = await supabase
        .from("profiles")
        .select("full_name, role, registration_no")
        .eq("id", user.id)
        .single();
      ...
      setAdminName(profile?.full_name || user.email?.split("@")[0] || "Administrator");

      // Unique Admin Code
      let code = profile?.registration_no;
      if (!code) {
        ...
        await supabase
          .from("profiles")
          .update({ registration_no: code })
          .eq("id", user.id);
      }
      setAdminCode(code);

      // 2. Load Teachers created with / linked to this admin's code
      // (Also loads all teachers if none explicitly linked yet)
      const { data: teacherProfiles } = await supabase
        .from("profiles")
        .select("id, full_name, email, registration_no")
        .eq("role", "teacher");

      // Filter teachers linked to this admin's code, or all teachers if this is the primary admin
      const linkedTeachers = (teacherProfiles || []).filter(
        (t) => !t.registration_no || t.registration_no === code
      );
      ...
      // 3. Load Classrooms for these teachers
      ...
      // 4. Load Classroom Members (Students)
      ...
      if (classroomIds.length > 0) {
        ...
        // Count quizzes conducted in these classrooms
      ...
      // 5. Load Enrolled Student Profiles
      const distinctStudentIds = [
        ...new Set(classMemberships.map((m) => m.student_id)),
      ];
      ...
      if (distinctStudentIds.length > 0) {
        ...
        const studentList: EnrolledStudent[] = (studentProfiles || []).map((s) => ({
          ...s,
      ...
      // 6. Load all registered students for the dropdown
  ...
  async function handleCreateTeacher(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(apiUrl("/admin/create-teacher"), {
        method: "POST",
  ...
  async function handleCreateClassroom(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(apiUrl("/admin/create-classroom"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  async function handleEnrollStudent(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(apiUrl("/admin/add-student"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
