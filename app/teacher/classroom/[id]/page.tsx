...
export default function ClassroomManagement() {
  ...
  async function handleEnrollStudent(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/teacher/enroll-student", {
        ...
        body: JSON.stringify({ classroomId, studentEmail: addStudentEmail.trim() }),
      });

      const data = await res.json();

      if (!res.ok) {
        setAddStudentMsg(data.error || "Failed to enroll student.");
        return;
      }
      ...
      setAddStudentMsg(data.message || "Student enrolled successfully.");
      setAddStudentEmail("");
      setTimeout(() => {
      ...
      setAddStudentMsg(err.message || "Failed to enroll student.");
    } finally {
  ...
  return (
    <main className="classroom-page">
      ...
      <section className="classroom-content">
        ...
        <div className="classroom-actions">
          ...
          <button
            className="action-card"
            onClick={() =>
              router.push(`/teacher/classroom/${classroom.id}/materials`)
            }
          >
            <div className="action-icon">
              <Upload size={24} />
            ...
            <div>
              ...
              <p>Upload notes, PDFs, slides, and study resources.</p>
            </div>
          </button>
        ...
        <section className="students-section">
          ...
          <div className="section-heading">
            ...
            <button className="secondary-btn" onClick={() => { setShowAddStudent(true); setAddStudentMsg(""); setAddStudentEmail(""); }}>
              <UserPlus size={18} />
              Add Student
            </button>
          </div>
      ...
      {/* Add Student Modal */}
      {showAddStudent && (
        <div className="teacher-modal-overlay" onClick={() => setShowAddStudent(false)}>
          <div className="teacher-modal" onClick={(e) => e.stopPropagation()}>
            <h2>Add Student to Classroom</h2>
            <p>Enter the email of an existing student to enroll them.</p>

            <form onSubmit={handleEnrollStudent}>
              <label>Student Email</label>
              <input
                type="email"
                placeholder="e.g. student@edunexa.com"
              ...
              {addStudentMsg && (
                <p style={{
                  ...
                  fontSize: "0.85rem",
                  marginTop: "4px",
                  background: addStudentMsg.includes("success") || addStudentMsg.includes("enrolled")
                    ? "#dcfce7" : "#fef2f2",
                  color: addStudentMsg.includes("success") || addStudentMsg.includes("enrolled")
                    ? "#166534" : "#991b1b",
                }}>
                  {addStudentMsg}
                </p>
              ...
              <div className="teacher-modal-actions">
                ...
                >
                  {addStudentLoading ? "Enrolling..." : "Enroll Student"}
                </button>
              </div>
            </form>
          </div>
